...
export function InlineExplainer({ children, subject, topic }: InlineExplainerProps) {
  ...
  useEffect(() => {
    if (!error) return;
    cleanDom();
    const errText = error.replace(/</g, "&lt;").replace(/>/g, "&gt;");
